...
for x in range(0, len(index)):
	if x == 0:
		tab[x:index[x]+1].write(sys.argv[2] + '_' + filenames[x]+ '.ecsv', format='ascii.ecsv')
	else:
		tab[index[x-1]+1:index[x]+1].write(sys.argv[2] + '_' + filenames[x]+ '.ecsv', format='ascii.ecsv')
tab[index[-1]+1:].write(sys.argv[2] + '_' + filenames[-1]+ '.ecsv', format='ascii.ecsv')
